#app-logic #javascript #nutritrack #source-code

# 01 — Логика приложения (`app.js`)

---

## Саммари

`app.js` — единственный JS-файл приложения NutriTrack. Он содержит всю клиентскую логику без каких-либо фреймворков или сборщиков.

**Что делает этот код:**

- **Управление состоянием** — хранит текущую вкладку, выбранную дату, выбранный продукт и цель профиля в простых переменных (`let`).
- **Работа с датами** — генерирует строки дат в формате `YYYY-MM-DD`, умеет сдвигать дату на произвольное количество дней и форматировать её на русском языке («Сегодня», «Вчера», число + месяц).
- **localStorage** — все данные сохраняются локально в браузере: дневник питания по ключам `bju_log_YYYY-MM-DD` и профиль пользователя по ключу `bju_profile`.
- **Расчёт макронутриентов** — пересчитывает БЖУ и калории из значений «на 100г» в значения «на указанный вес порции»; суммирует все записи за день.
- **Вкладка «Дневник»** — рендерит SVG-кольца прогресса (Белки / Жиры / Углеводы / Калории), карточку итогов и список съеденных продуктов. Обрабатывает модальное окно добавления продукта (поиск по базе + ручной ввод).
- **Вкладка «Профиль»** — заполняет и сохраняет форму профиля, вычисляет ИМТ, считает стрик и среднее ккал за 7 дней, строит столбчатый график через Chart.js.
- **Вкладка «Нутрициолог»** — показывает совет дня, анализирует рацион за 7 дней по 7 параметрам (белок, калории, жиры, овощи, рыба, разнообразие, углеводы) и формирует карточки-рекомендации. Отвечает на вопросы пользователя через keyword-matching по массиву `NUTRI_ANSWERS`.
- **Инициализация** — при загрузке DOM открывает вкладку «Дневник» и вешает обработчик закрытия модального окна по клику на оверлей.
